"""Full 48 h lossless (blosc) sort as a SINGLE block with MountainSort5 scheme 2
and a 1 h (3600 s) uniform training window. Reports wall time and compute footprint
(peak USS/RSS/CPU + peak /nvme usage), split into the one-time materialize phase and
the sort phase. Counterpart to 24_sort_12h_blocks_1h_train.py, which sorts the same
recording with scheme-3 12 h blocks.

"Single 48 h block" == scheme 2: one classifier is trained on a single 1 h window
(uniformly sampled across the recording) and applied to the whole 48 h in one pass,
with no per-block re-training. This is the large-block limit of the block-duration
sweep (see 21_/22_).

The ms5 int32 detect_spikes ceiling (~19.9 h = 71,583 s; see
SORTING_COMPARISON_FINDINGS.md) is NOT hit here, despite the 48 h length:
- training detection runs on a fresh NumpyRecording of just the 1 h window
  (~108 M samples << 2^31);
- phase-2 classification iterates the full 48 h in ~833 s chunks (ceil(100e6/4 ch)),
  each detect_spikes call sees chunk-local times and the global offset is added in
  int64 (mountainsort5/schemes/sorting_scheme2.py).
The earlier 21.3 h crash came from a >19.9 h *training* window, not the recording
length. No ms5 / SpikeInterface patching is needed.
...
SHARED_CACHE = SR / "_singleblock_cmr_cache"
OUT = SR / "blosc-scheme2-train3600s"
DISK_FLOOR_GB = 1800
SEED = 42
TRAIN_S = 3600       # 1 h training window (uniform), applied to the whole 48 h
# scheme="2" => single block; scheme3_block_duration_sec (sort_store default) is
# ignored by ms5 under scheme 2, so we don't set it.
BASE = dict(scheme="2", cmr="global", whitening_seed=SEED, materialize_n_jobs=96, sort_n_jobs=5)
...
summary = {"block_s": "single_48h_scheme2", "train_s": TRAIN_S, "seed": SEED,
           "sort_n_jobs": BASE["sort_n_jobs"],
...
# ---- Phase 2: sort full 48 h as a single scheme-2 block / 1 h training (pure sort, sampled) ----
print(f"\n=== [{time.strftime('%T')}] sorting full 48 h: scheme 2 (single block), train={TRAIN_S}s (1 h) ===", flush=True)
sampler = TreeSampler(interval=1.0)
start_free = shutil.disk_usage("/nvme").free
sampler.start()
t0 = time.perf_counter()
...
(SR / "sorting_singleblock_scheme2_summary.json").write_text(json.dumps(summary, indent=2))
shutil.rmtree(SHARED_CACHE, ignore_errors=True)
print(f"[{time.strftime('%T')}] removed shared cache", flush=True)

print("\n--- 48 h SORT @ single block / scheme 2 / 1 h training ---", flush=True)
