"""Build a SortingAnalyzer (Zarr) for the full-48 h, single-block / scheme-2 sort.

Pairs the aggregated sorting at
``sortings_seed42_pcafix/blosc-scheme2-train3600s/aggregated`` (produced by
``27_sort_48h_singleblock_scheme2.py``) with the SAME preprocessed recording it was
sorted on -- bandpass(300-6000 Hz) + global CMR, float32 -- and computes the standard
postprocessing + metrics extension suite into ``analyzer.zarr``. Counterpart to
``25_build_analyzer_12hblock_train1h.py`` for the 12 h-block scheme-3 sort.
...
SORT_DIR = SR / "blosc-scheme2-train3600s"
...
def main():
    ...
    (SR / "analyzer_singleblock_scheme2_summary.json").write_text(json.dumps(summary, indent=2))

    print("\n--- 48 h SortingAnalyzer @ single block / scheme 2 / 1 h training ---", flush=True)
